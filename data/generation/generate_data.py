...
class dataset_generator:
    ...
    def __init__(
        ...
        self.folder = (
            f"data_p{nb_nodes}_e{nb_nodes * expected_degree}_n{nb_points}_{suffix}"
        )
    ...
    def _initialize_dag(self):
        ...
        self.generator = gen.DagGenerator(
            self.mechanism,
            noise=self.noise,
    ...
    def _pick_targets(self, nb_max_iteration=100000):
        ...
        if self.max_nb_target == 1:
            intervention = np.random.choice(
                self.nb_nodes, self.nb_interventions, replace=False
            )
            ...
            while not_correct and i < nb_max_iteration:
                ...
                for _ in range(self.nb_interventions):
                    ...
                    intervention = np.random.choice(
                        self.nb_nodes, nb_targets, replace=False
                    )
    ...
    def generate(self, intervention=False):
        ...
        if intervention:
            ...
            for j in range(nb_env):
                ...
                dataset, _ = self.generator.intervene(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--noise", type=str, default="variable_gaussian", help="Distribution of noises"
    )
    parser.add_argument(
        "--noise-coeff", type=float, default=0.4, help="Noise coefficient"
    )
    parser.add_argument(
        "--nb-nodes", type=int, default=10, help="Number of nodes in the DAGs"
    )
    parser.add_argument(
        "--expected-degree",
        type=float,
    ...
    parser.add_argument(
        "--nb-dag", type=int, default=10, help="Number of DAGs to generate dataset from"
    )
    parser.add_argument(
        "--nb-points", type=int, default=1000, help="Number of points per dataset"
    )
    ...
    generator = dataset_generator(
        arg.mechanism,
        arg.initial_cause,
        arg.intervention_type,
        arg.struct_interv_distr,
        arg.noise,
        arg.noise_coeff,
        arg.nb_nodes,
        arg.expected_degree,
    ...
    args_dict = vars(arg)
    with open(f"{generator.folder}/dataset_params.txt", "w") as fp:
